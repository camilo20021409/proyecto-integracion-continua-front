...
async function obtenerPreguntaAleatoria() {
  try {
    ...
    const response = await fetch("https://proyectpoliback.onrender.com");
    ...
    document.getElementById("enunciado").textContent =
      preguntaAleatoria.acertijo;
    document.getElementById("opcion1").textContent =
      preguntaAleatoria.opciones.A;
    document.getElementById("opcion2").textContent =
      preguntaAleatoria.opciones.B;
    document.getElementById("opcion3").textContent =
      preguntaAleatoria.opciones.C;
    document.getElementById("opcion4").textContent =
      preguntaAleatoria.opciones.D;
  } catch (error) {
...
function verificarRespuesta(opcion) {
  const respuestaSeleccionada = document.getElementById(
    `opcion${opcion}`
  ).textContent;
  const respuestaCorrecta =
    preguntaAleatoria.opciones[preguntaAleatoria.respuesta_correcta];

  if (respuestaSeleccionada === respuestaCorrecta) {
    contadorRespuestasCorrectas++;
    document.getElementById("contadorCorrectas").textContent =
      contadorRespuestasCorrectas;
  } else {
    contadorRespuestasIncorrectas++;
    document.getElementById("contadorIncorrectas").textContent =
      contadorRespuestasIncorrectas;
  }
